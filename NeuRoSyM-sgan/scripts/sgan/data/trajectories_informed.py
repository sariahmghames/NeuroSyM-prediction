...
class TrajectoryDataset(Dataset): 
    ...
    def __init__(
            ...
            for idx in range(0, num_sequences * self.skip + 1, skip):
                ...
                for _, ped_id in enumerate(peds_in_curr_seq): 
                    ...
                    if pad_end - pad_front != self.seq_len:
                        continue
                    curr_ped_seq = np.transpose(curr_ped_seq[:, 2:]) # (2: are x and y)
                    ...
                    num_peds_considered += 1 # the num of peds that appear in all the frames considered for the seq len 
                ...
                if (self.repeat <= num_peds_considered and num_peds_considered!=1):
                    curr_seq_weight = np.ones((num_peds_considered*num_peds_considered, 2, self.seq_len)) 
        ...
        seq_list_weight = np.concatenate(seq_list_weight, axis=0) 
        ...
        self.obs_traj_weight = torch.from_numpy(seq_list_weight[:, :, :self.obs_len]).type(torch.float) 
