...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        neighborhood_size=2.0, grid_size=8, embedding_dim_cvar1 = 64, embedding_dim_cvar2 = 64, embedding_dim_cvar3 = 64
    ):
        ...
        if pool_every_timestep:
            if pooling_type == 'pool_net':
                self.pool_net = PoolHiddenNet_g(
                    ...
                    dropout=dropout,
                    obs_len = obs_len,
                    embedding_dim_cvar1 = embedding_dim_cvar1,
                    embedding_dim_cvar2 = embedding_dim_cvar2,
                    embedding_dim_cvar3 = embedding_dim_cvar3
        ...
        # TO BE CONTINUED
        self.spatial_embedding_cvar2 = nn.Linear(1, embedding_dim_cvar2)
        self.spatial_embedding_cvar3 = nn.Linear(2, embedding_dim_cvar3)

        self.hidden2pos = nn.Linear(h_dim, 2) # hidden to positions (outputs)

    def forward(self, last_pos, last_pos_rel, state_tuple, seq_start_end, traj, traj_weight, traj_causal_inf):
        """
        Inputs:
        ...
        for _ in range(self.seq_len):
            ...
            if self.pool_every_timestep: # every timestep predicted for the decoder output
                ...
                pool_h = self.pool_net(decoder_h, seq_start_end, curr_pos, traj, traj_weight, traj_causal_inf, self.mlp_pre_pool_dim_0) # pool decoder hidden , as the encoder pooling mechanism 
                decoder_h = torch.cat(
...
class PoolHiddenNet_g(nn.Module): # pooling hidden state of all people present in scene. encoder output and decoder output
    ...
    def __init__(
        ...
        activation='relu', batch_norm=True, dropout=0.0, obs_len = 8, embedding_dim_cvar1 = 64, embedding_dim_cvar2 = 64, embedding_dim_cvar3 = 64):
        ...
        self.embedding_dim = embedding_dim
        self.embedding_dim_cvar1 = embedding_dim_cvar1
        self.embedding_dim_cvar2 = embedding_dim_cvar2
        self.embedding_dim_cvar3 = embedding_dim_cvar3

        #self.mlp_pre_dim = (self.obs_len * embedding_dim) + h_dim # 1st mlp in fig 3 (pooling module)
        #self.mlp_pre_dim =  h_dim # 1st mlp in fig 3 (pooling module)
        self.mlp_pre_dim = (self.obs_len * embedding_dim) + h_dim  + ((self.obs_len) * self.embedding_dim_cvar1) + ((self.obs_len-1) * self.embedding_dim_cvar2) + ((self.obs_len-1) * self.embedding_dim_cvar3)# 1st mlp in fig 3 (pooling module)
        ...
        # TO BE CONTINUED
        self.cvar1_embedding = nn.Linear(1*(self.obs_len), self.embedding_dim_cvar1*(self.obs_len))
        self.cvar2_embedding = nn.Linear(1*(self.obs_len-1), self.embedding_dim_cvar2*(self.obs_len-1))
        self.cvar3_embedding = nn.Linear(2*(self.obs_len-1), self.embedding_dim_cvar3*(self.obs_len-1))
    ...
    @profile
    def forward(self, h_states, seq_start_end, end_pos, traj, traj_weight, traj_causal_inf, traj_cvars, mlp_pre_pool_dim_0):
        """
        Inputs:
        ...
        traj_cvar1 = traj_cvars[0]
        traj_cvar1 = traj_cvar1.permute(1,0)
        traj_cvar2 = traj_cvars[1]
        traj_cvar2 = traj_cvar2.permute(1,0)
        traj_cvar3 = traj_cvars[2]
        traj_cvar3 = traj_cvar3.permute(1,0,2)
        

        traj_cinf_cvar1 = traj_causal_inf[0]
        traj_cinf_cvar1 = traj_cinf_cvar1[:self.obs_len, :, :].permute(1, 0, 2) 
        traj_cinf_cvar2 = traj_causal_inf[1]
        traj_cinf_cvar2 = traj_cinf_cvar2[:self.obs_len, :, :].permute(1, 0, 2) # (33, 8, 2)
        traj_cinf_cvar3 = traj_causal_inf[2]
        traj_cinf_cvar3 = traj_cinf_cvar3[:self.obs_len, :, :].permute(1, 0, 2) 
        ...
        for _, (start, end) in enumerate(seq_start_end): # start and end are indices
            ...
            #print("curr_pose_history=", curr_pose_history)
            ...
            ## Comment out below if 2 cvars and qtc are used together with np.max()
            #traj_causal_inf_cvar1 = traj_cinf_cvar1[start:end].repeat(1, 1, int(self.embedding_dim/traj_weight.shape[2]))
            traj_causal_inf_cvar1 = traj_cinf_cvar1[start:end]
            traj_causal_inf_cvar1 = traj_causal_inf_cvar1.repeat(1, 1, int(self.embedding_dim_cvar1/traj_causal_inf_cvar1.shape[2])) ## CHECK SHAPE OF TRAJ_CAUSLAL_INF
            traj_causal_inf_cvar1 = traj_causal_inf_cvar1.repeat(num_ped, 1, 1)
            traj_causal_inf_cvar1 = torch.reshape(traj_causal_inf_cvar1, (traj_causal_inf_cvar1.size(0), traj_causal_inf_cvar1.size(1)*traj_causal_inf_cvar1.size(2)))


            traj_causal_inf_cvar2 = traj_cinf_cvar2[start:end]
            traj_causal_inf_cvar2 = traj_causal_inf_cvar2[:, :(self.obs_len-1),:]
            traj_causal_inf_cvar2 = traj_causal_inf_cvar2.repeat(1, 1, int(self.embedding_dim_cvar2/traj_causal_inf_cvar2.shape[2])) ## CHECK SHAPE OF TRAJ_CAUSLAL_INF
            traj_causal_inf_cvar2 = torch.reshape(traj_causal_inf_cvar2, (traj_causal_inf_cvar2.size(0), traj_causal_inf_cvar2.size(1)*traj_causal_inf_cvar2.size(2)))
                    

            traj_causal_inf_cvar3 = traj_cinf_cvar3[start:end]
            traj_causal_inf_cvar3 = traj_causal_inf_cvar3[:, :(self.obs_len-1),:]
            traj_causal_inf_cvar3 = traj_causal_inf_cvar3.repeat(1, 1, int(self.embedding_dim_cvar3/traj_causal_inf_cvar3.shape[2])) ## CHECK SHAPE OF TRAJ_CAUSLAL_INF
            traj_causal_inf_cvar3 = torch.reshape(traj_causal_inf_cvar3, (traj_causal_inf_cvar3.size(0), traj_causal_inf_cvar3.size(1)*traj_causal_inf_cvar3.size(2)))


            curr_traj_cvar1  = traj_cvar1[start:end]
            curr_traj_cvar1 = curr_traj_cvar1.repeat(num_ped, 1)
            #curr_traj_cvar1 = torch.reshape(curr_traj_cvar1, (curr_traj_cvar1.size(0), curr_traj_cvar1.size(1)*curr_traj_cvar1.size(2)))

            curr_traj_cvar2  = traj_cvar2[start:end]
            #curr_traj_cvar2 = curr_traj_cvar2.repeat(num_ped, 1, 1)

            curr_traj_cvar3  = traj_cvar3[start:end]
            curr_traj_cvar3 = curr_traj_cvar3.repeat(num_ped, 1, 1)
            curr_traj_cvar3 = torch.reshape(curr_traj_cvar3, (curr_traj_cvar3.size(0), curr_traj_cvar3.size(1)*curr_traj_cvar3.size(2)))

            

            #curr_rel_hist_embedding  = self.spatial_embedding(curr_rel_pos) # time (t-time_history) 
            
            ## Comment out below if no qtc-informed network
            #curr_rel_hist_embedding  = self.spatial_embedding(curr_rel_pos_history[:, 0, :]) # time (t-time_history) 
            #curr_rel_hist_embedding = traj_weit[:, 0, :].cuda() * curr_rel_hist_embedding # we can even put this 1st interaction weight to 0 to consider only weight at end-pose interaction
            #for h in range(1,curr_rel_pos_history.shape[1]):
            #    curr_rel_embedding  = traj_weit[:, h, :].cuda() * self.spatial_embedding(curr_rel_pos_history[:, h, :]) # input to 1st mlp layer of pooling mechanism, # (4,16)
            #    curr_rel_hist_embedding  = torch.cat([curr_rel_hist_embedding , curr_rel_embedding], dim=1) # hidden states which is output form encoder/decoder
            ...
            #curr_cvar1_history = []
            #curr_cvar1_history_nID = []
            #curr_cvar2_history = []  ## dtheta with closest neighbor
            #freq = 2.5
            #dt = 1/freq


            curr_cvar2_eval = torch.isnan(curr_traj_cvar2)



            if torch.any(curr_cvar2_eval):
                #print("some neighbors are sometimes still or the target person is sometimes still !!!!!!!!!!!!!!!!!!!!!!!!!!")
                ind_nan_cvar2 = torch.where(curr_cvar2_eval == True)
                for sub_rows in range(ind_nan_cvar2[0].size(-1)):
                    #print("sub rows =", sub_rows)
                    #curr_cvar2_history_[ind,:] = torch.zeros((curr_pose_history.size(1))-1)
                    curr_traj_cvar2[ind_nan_cvar2[0][sub_rows], ind_nan_cvar2[1][sub_rows]] = 0
                    traj_causal_inf_cvar2[ind_nan_cvar2[0][sub_rows], ind_nan_cvar2[1][sub_rows]] = 0
                    if torch.all(curr_traj_cvar3[ind_nan_cvar2[0][sub_rows], ind_nan_cvar2[1][sub_rows]]):
                        # then the neighbor is still and not the target person
                        traj_causal_inf_cvar3[ind_nan_cvar2[0][sub_rows], :] = 0

                #_curr_cvar2_history_ = torch.cat(curr_cvar2_history, axis = 0) # 2nd  
                _curr_cvar2_history_ = curr_traj_cvar2.repeat(num_ped,1)
                traj_causal_inf_cvar2 = traj_causal_inf_cvar2.repeat(num_ped,1)
                traj_causal_inf_cvar3 = traj_causal_inf_cvar3.repeat(num_ped,1)

                _curr_cvar2_history_ = _curr_cvar2_history_.to(device="cuda")  
                curr_cvar1_history_embedding = traj_causal_inf_cvar1 * self.cvar1_embedding(curr_traj_cvar1)           
                curr_cvar2_history_embedding = traj_causal_inf_cvar2 * self.cvar2_embedding(_curr_cvar2_history_)
                curr_cvar3_history_embedding = traj_causal_inf_cvar3 * self.cvar3_embedding(curr_traj_cvar3)
                mlp_h_input = torch.cat([curr_rel_hist_embedding, curr_cvar1_history_embedding, curr_cvar2_history_embedding, curr_cvar3_history_embedding, curr_hidden_1], dim=1)
...
class PoolHiddenNet_d(nn.Module): # pooling hidden state of all people present in scene. encoder output and decoder output
    ...
    def forward(self, h_states, seq_start_end, end_pos, traj, traj_weight, mlp_pre_pool_dim_0):
        ...
        pred_traj_gt = pred_traj_gt.permute(1,0,2)
        #traj_weight = traj_weight.permute(1, 0, 2)
        #obs_traj_weight = traj_weight[:self.obs_len, :, :].permute(1,0,2)
        #obs_traj_weight = obs_traj_weight.permute(1,0,2)
...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        pool_every_timestep=True, dropout=0.0, bottleneck_dim=1024,
        activation='relu', batch_norm=True, neighborhood_size=2.0, grid_size=8, embedding_dim_cvar1 = 64, embedding_dim_cvar2 = 64, embedding_dim_cvar3 = 64
    ):
        ...
        if pooling_type == 'pool_net':
            self.pool_net_g = PoolHiddenNet_g(
                ...
                batch_norm=batch_norm,
                obs_len = obs_len, 
                embedding_dim_cvar1 = self.embedding_dim_cvar1,
                embedding_dim_cvar2 = self.embedding_dim_cvar2,
                embedding_dim_cvar3 = self.embedding_dim_cvar3
            )
    ...
    def forward(self, traj, traj_rel, traj_weight,  traj_causal_inf, traj_cvars, seq_start_end, user_noise=None):
        """
        Inputs:
        - obs_traj: Tensor of shape (obs_len, batch, 2)
        ...
        if self.pooling_type:
            end_pos = obs_traj[-1, :, :]
            pool_h = self.pool_net_g(final_encoder_h, seq_start_end, end_pos, traj, traj_weight, traj_causal_inf, traj_cvars, mlp_pre_pool_dim_0) # pool hidden state of encoder
        ...
        decoder_out = self.decoder(
            ...
            last_pos_rel,
            state_tuple,
            seq_start_end, obs_traj, traj_weight, traj_causal_inf
        )
...
class TrajectoryDiscriminator(nn.Module):
    def __init__(
        ...
        d_type='local', embedding_dim_cvar1 = 64, embedding_dim_cvar2 = 64,embedding_dim_cvar3 = 64):
    ...
    def forward(self, traj, traj_rel, traj_weight, traj_causal_inf, traj_cvars, seq_start_end=None):
        """
        Inputs:
