...
class PoolHiddenNet_g(nn.Module): # pooling hidden state of all people present in scene. encoder output and decoder output
    """ Pooling module as proposed in our neurosym approach paper for sgan"""
    def __init__(
        ...
        self.mlp_pre_dim = (self.obs_len * embedding_dim) + h_dim
        #self.mlp_pre_dim =  h_dim 
        mlp_pre_pool_dims = [self.mlp_pre_dim, 512, bottleneck_dim] 
    ...
    def repeat3d(self, tensor, num_reps):
        ...
        tensor = tensor.repeat(num_reps, 1, 1) 
        tensor = tensor.view(-1, col_len, depth_len) 
        return tensor
    ...
    def forward(self, h_states, seq_start_end, end_pos, traj, traj_weight, mlp_pre_pool_dim_0):
        ...
        traj_weight = traj_weight[:self.obs_len, :, :].permute(1, 0, 2)
        ...
        for _, (start, end) in enumerate(seq_start_end): 
            ...
            curr_rel_hist_embedding  = self.spatial_embedding(curr_rel_pos_history[:, 0, :]) 

            curr_rel_hist_embedding = traj_weit[:, 0, :] * curr_rel_hist_embedding
            for h in range(1,curr_rel_pos_history.shape[1]):
                curr_rel_embedding  = traj_weit[:, h, :]* self.spatial_embedding(curr_rel_pos_history[:, h, :]) 
                curr_rel_hist_embedding  = torch.cat([curr_rel_hist_embedding , curr_rel_embedding], dim=1) 
...
class PoolHiddenNet_d1(nn.Module): # pooling hidden state of all people present in scene. encoder output and decoder output
    """ Pooling module similar to the generator one - trial - not used at the end """
    def __init__(
        self, embedding_dim=64, h_dim=64, mlp_dim=1024, bottleneck_dim=1024,
        ...
        self.mlp_pre_dim = ((self.obs_len+self.pred_len) * embedding_dim) + h_dim 
        #self.mlp_pre_dim =  (self.obs_len * embedding_dim) + h_dim 
        mlp_pre_pool_dims = [self.mlp_pre_dim, 512, bottleneck_dim]
...
class PoolHiddenNet_d(nn.Module): # pooling hidden state of all people present in scene. encoder output and decoder output
    """ Pooling module as proposed in SGAN paper"""
    def __init__(
        self, embedding_dim=64, h_dim=64, mlp_dim=1024, bottleneck_dim=1024,
        activation='relu', batch_norm=True, dropout=0.0, obs_len=8, pred_len=8
    ):
        ...
        self.mlp_pre_dim =  embedding_dim + h_dim 
        mlp_pre_pool_dims = [self.mlp_pre_dim, 512, bottleneck_dim] 
...
class TrajectoryGenerator(nn.Module):
    ...
    def forward(self, traj, traj_rel, traj_weight,  seq_start_end, user_noise=None):
        ...
        if self.pooling_type:
            end_pos = obs_traj[-1, :, :]
            pool_h = self.pool_net_g(final_encoder_h, seq_start_end, end_pos, traj, traj_weight, mlp_pre_pool_dim_0) 
