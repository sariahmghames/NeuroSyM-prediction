...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        # TO BE CONTINUED
        #self.spatial_embedding_cvar1 = nn.Linear(1, embedding_dim_cvar1)
...
class PoolHiddenNet_g(nn.Module): # pooling hidden state of all people present in scene. encoder output and decoder output
    ...
    def __init__(
        ...
        self.mlp_pre_dim = (self.cnd_lag * embedding_dim) + h_dim  # 1st mlp in fig 3 (pooling module)
        ...
        self.spatial_embedding = nn.Linear(2, embedding_dim)
        # TO BE CONTINUED
        #self.cvar1_embedding = nn.Linear(1*(self.causal_lag), self.embedding_dim_cvar1*(self.causal_lag))
        #self.cvar2_embedding = nn.Linear(1*(self.causal_lag), self.embedding_dim_cvar2*(self.causal_lag))
        #self.cvar3_embedding = nn.Linear(2*(self.causal_lag), self.embedding_dim_cvar3*(self.causal_lag))
    ...
    def forward(self, h_states, seq_start_end, end_pos, traj, traj_weight, mlp_pre_pool_dim_0):
        ...
        ##traj_weight = [item[:self.obs_len, :, :] for item in traj_weight]
        #traj_weight = traj_weight[:, :, :].permute(1, 0, 2)
        

        # traj_cinf_cvar1 = traj_causal_inf[0]
        # traj_cinf_cvar1 = traj_cinf_cvar1[-2:, :, :].permute(1, 0, 2) 
        # traj_cinf_cvar2 = traj_causal_inf[1]
        # traj_cinf_cvar2 = traj_cinf_cvar2[-2:, :, :].permute(1, 0, 2) # (33, 8, 2)
        # traj_cinf_cvar3 = traj_causal_inf[2]
        # traj_cinf_cvar3 = traj_cinf_cvar3[-2:, :, :].permute(1, 0, 2) 
        ...
        for _, (start, end) in enumerate(seq_start_end): # start and end are indices
            ...
            #print("curr_pose_history=", curr_pose_history.shape)
            ...
            traj_weiit = traj_weiit[:, -1, :]
            traj_weit = traj_weiit.repeat(1, int(self.embedding_dim/traj_weiit.shape[1]))
            

            curr_rel_hist_embedding  = traj_weit.cuda() * self.spatial_embedding(curr_rel_pos) # time (t-time_history) 
...
class PoolHiddenNet_d1(nn.Module): # pooling hidden state of all people present in scene. encoder output and decoder output
    """ Pooling module as proposed in our paper"""
    def __init__(
        self, embedding_dim=64, h_dim=64, mlp_dim=1024, bottleneck_dim=1024,
    ...
    def forward(self, h_states, seq_start_end, end_pos, traj, traj_weight, mlp_pre_pool_dim_0):
        ...
        #pred_traj_weight = traj_weight[self.obs_len:, :, :].permute(1,0,2)
        #pred_traj = pred_traj.permute(1,0,2)
        #print("traj weight shape=", traj_weight.shape)
        ...
        for _, (start, end) in enumerate(seq_start_end): # start and end are indices
            ...
            curr_rel_hist_embedding  = self.spatial_embedding(curr_rel_pos_history[:, 0, :]) # time (t-time_history) 
            #print(obs_traj_weit[:, 0, :].shape) # 2, 16
            #print(curr_rel_hist_embedding.shape)
            curr_rel_hist_embedding = traj_weit[:, 0, :] * curr_rel_hist_embedding
            for h in range(1,curr_rel_pos_history.shape[1]):
                curr_rel_embedding  = traj_weit[:, h, :]* self.spatial_embedding(curr_rel_pos_history[:, h, :]) # input to 1st mlp layer of pooling mechanism, # (4,16)
                curr_rel_hist_embedding  = torch.cat([curr_rel_hist_embedding , curr_rel_embedding], dim=1) # hidden states which is output form encoder/decoder
            #print("curr rel hist embed dim =", curr_rel_hist_embedding.shape) # 5, 512 cz 64*8 = 512
            mlp_h_input = torch.cat([curr_rel_hist_embedding, curr_hidden_1], dim=1)
            #print("mlp_h_input dim = ", mlp_h_input.shape)
